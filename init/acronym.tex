...
\acro{ic}[CI]{Circuito Integrado}
\acrodefplural{ic}[CIs]{Circuitos Integrados}
\acro{cts}[CTS]{\textit{Clock Tree Synthesis}}

% D
\acro{dme}[DME]{\textit{Deferred-Merge Embedding}}
% \acro{dod}[DOD]{Desenvolvimento Orientado a Dados - \textit{Data-Oriented Design}}
\acro{dod}[DOD]{\textit{Data-Oriented Design}}

% E
% \acro{ecl}[ECL]{Laboratório de ComputaçãSo Embarcada - \textit{Embedded Computing Lab}}
\acro{ecl}[ECL]{\textit{Embedded Computing Lab}}
\acro{eda}[EDA]{\textit{Electronic Design Automation}}

% F
\acro{fm}[FM]{Fiduccia-Mattheyses}

% G
\acro{ge}[GE]{\textit{Grammatical Evolution}}

% H
\acro{hdl}[HDL]{\textit{Hardware Description Language}}
\acro{hpc}[HPC]{\textit{High-Performance Computing}}
\acro{hpwl}[HPWL]{\textit{Half-Perimeter Wirelength}}
% I
\acro{ilp}[ILP]{Programação Linear Inteira}
\acro{itdp}[ITDP]{\textit{Incremental Timing-Driven Placement}}

% J

% K
\acro{kl}[KL]{Kernighan-Lin}
% L
...
% O
% \acro{ood}[OOD]{Desenvolvimento Orientado a Objetos --- \textit{Object-Oriented Design}}
\acro{ood}[OOD]{\textit{Object-Oriented Design}}

% P
\acro{pdg}[PDG]{\textit{Program Dependence Graph}}
% Q

% R
\acro{rgm}[RGM]{\textit{Recursive Geometric Matching}}
\acro{rtl}[RTL]{\textit{Register Transfer Level}}
\acro{rsmt}[RSMT]{\textit{Rectilinear Steiner Minimum Tree}}

% S
\acro{sa}[SA]{\textit{Simulated Annealing}}
\acro{simd}[SIMD]{\textit{Single Instruction Multiple Data}}
\acro{sisd}[SISD]{\textit{Single Instruction Single Data}}
\acro{soa}[SoA]{\textit{Structure of Arrays}}
\acro{sta}[STA]{\textit{Static Timing Analysis}}

% T
\acro{trg}[TRG]{\textit{Temporal-Relation Graph}}
% U
...
\acro{sea}[SEA]{Algoritmo de Eliminações Sucessivas - \textit{Successive Elimination Algorithm}}
